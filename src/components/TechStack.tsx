...
function OrbitItem({ icon: Icon, name, color, radius, startAngle, duration, image }: {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className="w-10 h-10 rounded-xl bg-white flex items-center justify-center shadow-lg border border-black/[0.03] transition-all duration-300 overflow-hidden"
...
export default function TechStack() {
  return (
    <section id="tech" className="py-20 relative overflow-hidden bg-white">
      <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-black/[0.03] to-transparent" />
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 65% 55% at 30% 55%, #f4f9ff 0%, transparent 80%)' }} />
      ...
      <div className="container mx-auto px-6 max-w-[1100px] relative z-10">
        ...
        >
          ...
          <p className="text-secondary max-w-xl mx-auto text-base font-medium opacity-70">
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-6 lg:gap-0">
          ...
          >
            {/* Guide rings */}
            <div className="absolute rounded-full border border-dashed border-black/[0.03] pointer-events-none"
              style={{ width: INNER_R * 2, height: INNER_R * 2, left: ORBIT_SIZE / 2 - INNER_R, top: ORBIT_SIZE / 2 - INNER_R }} />
            <div className="absolute rounded-full border border-dashed border-black/[0.02] pointer-events-none"
            ...
            <motion.div
              ...
              viewport={{ once: true }}
              className="absolute z-20 w-24 h-24 rounded-full bg-white flex items-center justify-center shadow-[0_0_40px_rgba(0,113,227,0.1)] border border-black/[0.02]"
          ...
          >
            ...
            <p className="text-secondary text-base font-medium leading-relaxed mb-10 opacity-70">
              Direct, real-time integration into the platforms you love. Sync data and trigger workflows instantly.
            </p>

            {/* Compact Chip Grid */}
            <div className="grid grid-cols-2 gap-2.5 pb-8 border-b border-black/[0.03] mb-8">
              {brandLogos.map((brand, i) => (
                <motion.div
                  ...
                  whileHover={{ backgroundColor: '#f9fbff', scale: 1.02 }}
                  className="flex items-center gap-2.5 bg-white px-3.5 py-2.5 rounded-lg border border-black/[0.05] shadow-sm transition-all duration-300 cursor-default"
                >
