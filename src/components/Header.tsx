  ...
  return (
    <header className="fixed top-0 w-full z-50 bg-white/80 backdrop-blur-xl border-b border-black/5">
      <div className="container mx-auto px-6 h-16 flex items-center justify-between">
        ...
        {/* Centered Nav */}
        <nav className="hidden md:flex gap-8 text-[11px] font-black uppercase tracking-widest text-secondary">
          {navLinks.map((link) => (
            <Link 
              key={link.name} 
              href={link.href} 
        ...
        <div className="flex items-center gap-4">
          <Link href="#" className="hidden md:block text-[13px] font-medium text-secondary hover:text-foreground transition-colors">
            Login
          </Link>
